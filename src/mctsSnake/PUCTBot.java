...
public class PUCTBot implements Bot {
    ...
    private PUCTController controller = new PUCTController();
	...
	try {
	    outBuffer =
		new BufferedWriter(new FileWriter("PUCTlog.txt"));
	} catch (IOException e){}
    }
	...
	Direction chosenDir = toDirection(finalMoves[PUCTController.ROW_MOVE], rowPrevCurDir[CUR]);
	try{
    ...
    public PUCTController getController(){return controller;}
    ...
    private void rollout(Snake snake, Snake opponent, Coordinate mazeSize,
			 Coordinate apple, PUCTController controller){
	int curDepth = 0;
	boolean defaultPolicy = false;
	boolean[] rowConstraints = new boolean[PUCTNode.NUM_OPTIONS];
	boolean[] colConstraints = new boolean[PUCTNode.NUM_OPTIONS];
	...
	int[] nextMove = new int[PUCTController.RETURN_SIZE];
	...
	for (int i = 0; i < PUCTNode.NUM_OPTIONS; i++)
	    defaultMoves.add(i);
	...
	while (curDepth < MAX_DEPTH) {
		...
		nextMove[PUCTController.ROW_MOVE] = greedyPolicy(rowDistances, rowConstraints);
		nextMove[PUCTController.COL_MOVE] = greedyPolicy(colDistances, colConstraints);
		nextMove[PUCTController.LEAF_NODE] = 1;
	    ...
	    // ----------- Run Game Step ------------
	    Direction rowDir = toDirection(nextMove[PUCTController.ROW_MOVE],
					   rowPrevCurDir[CUR]);
	    Direction colDir = toDirection(nextMove[PUCTController.COL_MOVE],
					   colPrevCurDir[CUR]);
	...
	int[] bestChoices = new int[PUCTNode.NUM_OPTIONS];
    ...
    */
	...
	for (int i = 0; i < PUCTNode.NUM_OPTIONS; i++){ // Iterate through forward, right, left
	...
	return numConstraints < PUCTNode.NUM_OPTIONS;
    }
    ...
    */
	...
	if (relDirection == PUCTController.FORWARD)
	    return baseDirection;
	if (relDirection == PUCTController.LEFT)
	    return matchDirection(baseDirection.dy, -1 * baseDirection.dx);
	if (relDirection == PUCTController.RIGHT)

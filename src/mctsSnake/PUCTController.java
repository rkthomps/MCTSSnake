...
/*
  ...
  except it uses PUCT to make its decisions.


  TODO: HAVENT LOOKED AT THIS FILE
 */
public class PUCTController {
    ...
    private PUCTNode rootNode;
    private PUCTNode curNode;

    public PUCTController() {
	this.rootNode = new PUCTNode();
	this.curNode = rootNode;
    }

    public PUCTController(PUCTNode rootNode){
	this.rootNode = rootNode;
	this.curNode = rootNode;
    }

    public PUCTNode getRoot(){return rootNode;}

    /*
      Change the root of the controller to be one of the root's children
     */
    public void moveRoot(int rowMove, int colMove){
	PUCTNode targetNode = rootNode.getChild(rowMove, colMove);
	if (targetNode == null)
	    targetNode = new PUCTNode();
	rootNode = targetNode;
	curNode = rootNode;
	...
	PUCTNode toReturn = rootNode.getBestSuccessor();
	int[] returnArr = new int[RETURN_SIZE];

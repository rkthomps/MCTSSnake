...
public class NaiveNode {
    ...
    private NaiveNode parent; // Which node is this node's parent?
    ...
    private NaiveNode[][] children = new NaiveNode[NUM_OPTIONS][NUM_OPTIONS];
    ...
    public NaiveNode(NaiveNode parent, int rowIndex, int colIndex){
	this.parent = parent;
    ...
    public NaiveNode getChild(int rowMove, int colMove){
    ...
    /*
      ...
      than size three.
     */
    public NaiveNode bestChild(boolean[] rowConstraints, boolean[] colConstraints,
	...
	for (int i = 0; i < expandQueue.size(); i++){
		...
		continue;
	    NaiveNode newNode = new NaiveNode(this, nodeRow, nodeCol);
	...
	double[][][] scoreVisits = getScoreVisit(validRows, validCols,
	...
	double[][] score = scoreVisits[0];
	double[][] visits = scoreVisits[1];

	double[] rowScores = Util.sumAcross(score);
	double[] colScores = Util.sumOver(score);
	double[] rowVisits = Util.sumAcross(visits);
	double[] colVisits = Util.sumOver(visits);

	double[] rowExpected = Util.divide(rowScores, rowVisits);
	double[] colExpected = Util.divide(colScores, colVisits);
    ...
    */
    public NaiveNode getBestSuccessor(){
	int[] validRows = new int[NUM_OPTIONS];
	int[] validCols = new int[NUM_OPTIONS];
	int payoffRows = 0;
	int payoffCols = 0;
     ...
     */
    private double[][][] getScoreVisit(int[] validRows, int[] validCols,
					int payoffRows, int payoffCols){
	double[][][] scoreVisit = new double[2][payoffRows][payoffCols];
	NaiveNode curChild;
	for (int i = 0; i < payoffRows; i++){
	    for (int j = 0; j < payoffCols; j++){
		curChild = children[validRows[i]][validCols[j]];
		scoreVisit[0][i][j] = curChild.getScore();
		scoreVisit[1][i][j] = curChild.getVisits();
	    }
	}
	return scoreVisit;
    }
